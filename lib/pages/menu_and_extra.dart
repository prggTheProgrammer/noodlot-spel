...
class HomePage extends GamePage {
  HomePage({super.key}) : super(
      ...
      [
        ...
        Button("Eindes", (context) => getEindes())
      ]
  );
  ...
  List<Widget> body(BuildContext context){
    return [
      Padding(padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 10), child: Text(title, style: Style.subtitle(context),)),
      Padding(padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 10), child: Text(story, style: Style.subtitle(context)))
    ];
  }
...
List<Button> eindes = [];
GamePage getEindes() => GamePage("Eindes", "In deze pagina vind je alle eindes die jij tot nu toe hebt gevonden. Als je op een einde klikt krijg je die einde te zien. Je hebt nu ${eindes.length} eindes van de 7 verzameld", eindes);
